...
function computeSilentCheckResult(graph: Graph, config: SpokesConfig, resolveWarnings: Diagnostic[]): CheckResult {
  ...
  const stats: CheckStats = { nodes: graph.nodes.size, edges: graph.edges.length, hubs, spokes, unmarked };
  return { version: 1, errors, warnings, stats };
}

function loadGraph(repoRoot: string): { graph: Graph; config: SpokesConfig; resolveWarnings: Diagnostic[] } {
...
function printResolveWarnings(resolveWarnings: Diagnostic[]): void {
...
export async function main(argv: string[]): Promise<number> {
  ...
  program
    ...
    .action((opts: { format: string; timestamp: boolean; out: string }) => {
      const { graph, config, resolveWarnings } = loadGraph(repoRoot);
  ...
  try {
    ...
    console.error(`internal error: ${(err as Error).stack ?? err}`);
