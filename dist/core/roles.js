...
function resolvePragma(fileContents) {
...
function resolveConfigRole(relPath, config) {
    for (const entry of config.roles) {
        if (globToRegExp(entry.glob).test(relPath))
            return entry.role;
    }
    return undefined;
}
/** Exports exactly one function, satisfying R4. */
export function resolveRole(fileContents, relPath, config) {
