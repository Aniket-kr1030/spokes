...
function moduleFile(baseDir, segments) {
    const p = segments.length > 0 ? join(baseDir, ...segments) : baseDir;
    if (segments.length > 0 && existsSync(`${p}.py`))
        return `${p}.py`;
 ...
 */
export function resolvePythonSpecifier(repoRoot, fromAbsPath, specifierText, fromImport) {
    let dots = 0;
    while (specifierText[dots] === '.')
        dots++;
    const rest = specifierText.slice(dots);
    const segments = rest.length > 0 ? rest.split('.') : [];
    const candidates = [segments];
    if (fromImport && segments.length > 0)
        candidates.push(segments.slice(0, -1));
    if (dots > 0) {
        ...
        for (const cand of candidates) {
            ...
            if (hit && hit !== fromAbsPath)
                return { kind: 'relative', absPath: hit };
        }
